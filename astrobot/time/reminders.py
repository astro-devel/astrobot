...
class Reminders(commands.Cog):
    ...
    async def send_reminder(
        ...
        embed = discord.Embed(title=reminder, color=bot.colors.white)
        await channel.send(
            f"<@!{member_id}>, here's your reminder! {apology}", embed=embed
        )
        ...
        db.session.execute(
            sqlalchemy.delete(db.Reminders)
            .where(db.Reminders.reminder_text == reminder)
    ...
    async def reminders(self, ctx, *, args: str = None):
        ...
        if args[0] == "list" or not args:
            if not self.bot.remindme_timers[ctx.author.id]:
                await ctx.send(
                    embed=discord.Embed(
                        title=f"{self.bot.custom_emojis.error} No active reminders found for {ctx.author}."
                    )
                )
                return
            ...
            for timer in self.bot.remindme_timers[ctx.author.id]:
                page_list.append(
                    discord.Embed(title=f"Active reminders for {ctx.author}")
                    .add_field(name="Reminder", value=timer.reminder_text)
                    .add_field(
                        name="Date",
            ...
            db.session.execute(
                sqlalchemy.delete(db.Reminders)
                .where(db.Reminders.reminder_text == item_to_delete.reminder_text)
                .execution_options(synchronize_session="fetch")
            )
            db.session.commit()
