...
class BugStatus:
    ...
    def init_from(self, status):
        if status == 'NEW': return self.new()
        elif status == 'ASSIGNED': return self.assigned()
        elif status == 'TRIAGE': return self.triage()
        elif status == 'OPEN': return self.open()
        elif status == 'CLOSED': return self.closed()
        elif status == 'PATCHED': return self.patched()
        else: return None
    def new(self): return self.Status(status="NEW", desciption="This is a fresh bug. No action has been taken yet.")
    def assigned(self): return self.Status(status="ASSIGNED", desciption="This bug has been assigned to a maintainer. It should be triaged soon.")
    def triage(self): return self.Status(status="TRIAGE", desciption="This bug is currently under review by the assigned individual.")
    def open(self): return self.Status(status="OPEN", desciption="This bug has been triaged, prioritized accordingly, and is being fixed in order of it's priority.")
    def closed(self): return self.Status(status="CLOSED", desciption="This bug has peen patched. The patch will be pushed to production with the next update.")
    def patched(self): return self.Status(status="PATCHED", desciption="This bug's lifecyle is over. The patch has now been pushed to production and is considered patched.")

class BugItem:

    __slots__ = ('timestamp', 'reporter', 'description', 'reproduction_steps', 'bot_version', 'screenshot_url', 'status', 'assigned_to', 'bug_id', 'priority', 'delete_at')
...
########################################    -assign and -deassign
    ...
    async def assign(self, ctx, bug_id, assignee: discord.Member=None):
        ...
        _tech_role: discord.Role = discord.utils.get(ctx.guild.roles, id=905942167963455498)
...
########################################    -delete
    ...
    async def delete(self, ctx, _bug_id):
        ...
        db_session.execute(
            sqlalchemy.delete(BugItem_DB).where(BugItem_DB.bug_id == _bug_id).execution_options(synchronize_session="fetch")
        )
        db_session.commit()
...
########################################    -update_priority
    ...
    async def update_priority(self, ctx, _bug_id: str, _new_priority: str=None):
        if not _new_priority:
            ...
            await ctx.send("i.e. 'low', 'normal', 'medium', 'high', 'crucial', 'emergency'")
            return
        ...
        await ctx.send(f"Successfully upated priority of '{_bug_id}' to '{_new_priority.upper()}'")
...
########################################    -update_status
    ...
    async def update_status(self, ctx, _bug_id: str, _new_status: str=None):
        if not _new_status:
            ...
            await ctx.send("i.e. 'new', 'assigned', 'triage', 'open', 'closed', 'patched'")
            return
        ...
        await ctx.send(f"Successfully upated status of '{_bug_id}' to '{_new_status.upper()}'")
...
########################################    -status_info
    ...
    async def status_info(self, ctx, _status: str):
        if _status.lower() == 'new': _r = BugStatus().new()
        elif _status.lower() == 'assigned': _r = BugStatus().assigned()
        elif _status.lower() == 'triage': _r = BugStatus().triage()
        elif _status.lower() == 'open': _r = BugStatus().open()
        elif _status.lower() == 'closed': _r = BugStatus().closed()
        elif _status.lower() == 'patched': _r = BugStatus().patched()
        ...
        if subcmd == 'new':
            for bug in self.bugs:
                if self.bugs[bug].status.status.lower() == 'new':
                    _list.append(f"{self.bugs[bug].bug_id} ({self.bugs[bug].description[0:25]})")
                    if len(_list) == 10:
                        break
            ctr = 1
            for _id in _list:
...
########################################    -track
    ...
    async def track(self, ctx, bug_id):
        ...
        embed = discord.Embed(
            ...
            name="**Priority:**",
            value=_bug_item.priority.level
        ).add_field(
            name="**Status:**",
            value=_bug_item.status.status
        ).add_field(
            name="**Reported (epoch time):**",
            value=_bug_item.timestamp
        ).add_field(
            name="**Reported by:**",
            value=_bug_item.reporter.mention
        ).add_field(
            name="**Bot version:**",
        ...
        if len(_bug_item.description) > 97:
            embed.add_field(
                name="**Description**",
                value=f"{_bug_item.description[0:97]}..."
            )
        else:
            embed.add_field(
                name="**Description**",
                value=_bug_item.description
            )
        if _bug_item.screenshot_url:
            embed.add_field(
                name="**Screenshot:**",
                value=_bug_item.screenshot_url
            )
...
###############################################################
    ...
    async def is_dmable(self, ctx):
        try:
            await ctx.author.send("Hello, I heard you want to report a bug! First, may I ask what version I was when this bug occured? If it's the current version, say 'current'.")
            return True
    ...
    async def report(self, ctx):
        ...
        if not _is_dmable:
            ...
            await _report_channel.send("Hello, I heard you want to report a bug! First, may I ask what version I was when this bug occured? If it's the current version, say 'current'.")
        ...
        await _report_channel.send("Thanks! Now, may I get a short description of the issue? (less than 1000 characters)")
        ...
        await _report_channel.send("What are the steps than can be taken to reproduce this issue? (i.e. \"1...2...3...\")")
        ...
        ##################### Get screenshot
        await _report_channel.send("Dope! Finally, do you have a screenshot of the issue? If so, just send them. If not, just say 'no'")
        ...
        if str(_screenshots) == 'no':
            await _report_channel.send("Ok, sounds good! Let me get this issue put in the system for you, and I'll be right back with your trackable Bug ID!")
        else:
            await _report_channel.send("Thanks! Let me get this issue put in the system for you, and I'll be right back with your trackable Bug ID!")
        bug_item = BugItem(
            reporter = _reporter,
...
-\t[ TECHNICIAN ] status [bug ID] [status]: update status for a given bug```''')
        ...
        # technician commands
            ...
            return (self.technician_commands['assign'], (args[1], await ctx.guild.fetch_member(re.sub("[<@!>]", "", args[2]))) if len(args) > 2 else [args[1]])
